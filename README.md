...
bun run sync-data     # regenerate src/data/generated/* from aoe4world data
```

The icon catalog and per-civ data (`src/data/generated/{icons,civData,pathMigration}.ts`)
are regenerated by `scripts/sync-aoe4-data.ts`. Re-run it whenever a new
patch ships or aoe4world's entity lists change — see `scripts/README.md`
for what it pulls and where assets land.
