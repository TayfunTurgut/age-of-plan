...
bun run build:catalog # regenerate src/data/iconCatalog.ts from aoe4world/data
```

The icon catalog (`src/data/iconCatalog.ts`) is regenerated by
`scripts/build-icon-catalog.ts`. Re-run it whenever assets are added or
aoe4world's entity lists change. The script asserts that every catalog path
resolves to a file under `public/assets/aoe4/` and bails on regression.
