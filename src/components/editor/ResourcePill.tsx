...
type Props = {
  ...
  delta?: number;
  /**
   * When true, the numeric input is replaced by a static `?` glyph and the
   * delta is suppressed. Used by the Builders pill in late-game steps.
   */
  unknown?: boolean;
  /**
   * If provided, renders a small toggle button next to the input that flips
   * `unknown` on/off. Only the Builders pill uses this today.
   */
  onUnknownToggle?: (next: boolean) => void;
};

const ResourcePillImpl = ({
  resource,
  value,
  onChange,
  delta,
