...
import type { BuildStep, BuildTag } from "@/types/buildOrder";

type Props = {
  step: BuildStep;
  civId: string;
  onUpdate: (tags: BuildTag[]) => void;
};
...
function StepTagsImpl({ step, civId, onUpdate }: Props) {
  const tags = step.tags ?? [];
  const [autoFocusId, setAutoFocusId] = useState<string | null>(null);

  const update = (i: number, patch: Partial<BuildTag>) => {
  ...
  const add = () => {
    const fresh: BuildTag = { id: newId(), unit: "", location: "" };
