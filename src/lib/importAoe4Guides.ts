...
function htmlToText(html: string): string {
  ...
  // `&amp;` must be decoded LAST so double-encoded entities (e.g. `&amp;lt;`,
  // an author's literal "&lt;") decode exactly one level, not two.
  out = out
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/&quot;/g, '"')
...
const RawAoe4StepSchema = z.looseObject({
...
const RawAoe4AgeGroupSchema = z.looseObject({
  age: z.number().nullish(),
...
const RawAoe4GuidesSchema = z.looseObject({
  title: z.string().nullish(),
  name: z.string().nullish(),
  civilization: z.string().nullish(),
  civ: z.string().nullish(),
  author: z.string().nullish(),
  user: z.looseObject({ name: z.string().nullish() }).nullish(),
  description: z.string().nullish(),
