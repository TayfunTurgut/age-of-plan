import { z } from "zod";
import type { BuildOrder, Resources } from "@/types/buildOrder";
...
/**
 * Apply a per-note text transform to a step's `notes` array. Returns the
 * new notes plus a `touched` flag so the caller can decide whether the
 * surrounding step needs a fresh reference. Returns `null` when `notes`
 * isn't an array — the caller is then responsible for skipping the step.
 */
const transformNoteTexts = (
  notes: unknown,
  guard: (text: string) => boolean,
  transform: (text: string) => string,
): { notes: { id: string; text: string }[]; touched: boolean } | null => {
  if (!Array.isArray(notes)) return null;
  let touched = false;
  const out = (notes as Array<{ id: string; text: string }>).map((n) => {
    if (!n || typeof n.text !== "string" || !guard(n.text)) return n;
...
const migrate = (input: unknown): { value: unknown; mutated: boolean } => {
  ...
  const nextSteps = bo.steps.map((stepRaw: unknown) => {
    ...
    // Migrate icon token syntax: @path.ext@ → {{path.ext}}.
    {
      ...
      if (r?.touched) {
        mutated = true;
        nextStep = { ...nextStep, notes: r.notes };
      }
    }
    ...
    // The lookup table is generated by scripts/sync-aoe4-data.ts.
    {
      const r = transformNoteTexts(
        ...
        (t) => t.includes("{{"),
        (t) =>
          t.replace(NOTE_TOKEN_RE, (whole, oldPath: string) => {
