...
test("reordering steps via keyboard persists across reload", async ({ page }) => {
  await seed(page, twoStepBuild("drag-kbd-1"));
  await page.goto("/build/drag-kbd-1/edit");

  const firstFood = page.getByRole("spinbutton", { name: "Food" }).first();
  await expect(firstFood).toHaveValue("5");

  // Pick up step 2 with Space, move it up with ArrowUp, drop with Space.
  const handle = page.getByRole("button", { name: "Drag step" }).nth(1);
  await handle.focus();
